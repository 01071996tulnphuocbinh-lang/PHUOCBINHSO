...
export const getOrganization = async (
    ...
    // Ép kiểu 'any' để vượt qua lỗi "Spread types may only be created from object types"
    const rawOrg = db.organization as any;
    
    const org: Organization = {
        ...rawOrg,
        officialAccounts: (rawOrg.officialAccounts || []).map((oa: any) => ({
            oaId: String(oa.id),
            name: String(oa.name),
            // Gán link avatar từ db.json vào đây
            avatar: oa.avatar || "https://stc-zaloprofile.zdn.vn/pc/v1/images/oa_default.png",
            description: oa.description || "Cơ quan nhà nước",
            follow: false,
        })),
    } as Organization;

    return new Promise(resolve => {
        setTimeout(() => resolve(org), 200);
    });
};
...
export const getFeedbacks = async (
    ...
    const feedbacks: Feedbacks = {
        ...
        feedbacks: (db.feedbacks.data as any[]).map((item: any) => ({
            ...item,
...
export const getInformationGuides = async (
    ...
    Promise.resolve({
        ...db.guidelines,
        informationGuides: (db.guidelines.data as any[]).map((item: any) => ({
            ...item,
            id: Number(item.id),
        })),
    });
...
export const searchProfiles = async (
    ...
    new Promise(resolve => {
        setTimeout(() => {
            resolve(
                (db.profiles as any[]).map((item: any) => ({
                    ...item,
                    dueDate: new Date(item.dueDate),
                    notifications: (item.notifications || []).map((noti: any) => ({
                        ...noti,
